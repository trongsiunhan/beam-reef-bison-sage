...
//#region node_modules/.nitro/vite/services/ssr/assets/proxy-check.functions-v4YH2c_y.js
...
var checkProxyFn = createServerFn({ method: "POST" }).validator(ProxyInput.extend({ timeoutMs: number().int().min(800).max(8e3).optional() })).handler(checkProxyFn_createServerFn_handler, async ({ data }) => {
	const { probeProxy } = await import("./proxy-tunnel.server-C0Jwv65i.mjs");
	return probeProxy({
		ip: data.ip,
		port: data.port,
		type: data.type
...
var probeUrlFn = createServerFn({ method: "POST" }).validator(SessionInput.extend({ url: string().min(1) })).handler(probeUrlFn_createServerFn_handler, async ({ data }) => {
	const { browseNavigate } = await import("./browser-engine.server-CGd-YOU2.mjs");
...
var browseClickFn = createServerFn({ method: "POST" }).validator(object({
	...
	const { browseClick } = await import("./browser-engine.server-CGd-YOU2.mjs");
...
var browseTypeFn = createServerFn({ method: "POST" }).validator(object({
	...
	const { browseType } = await import("./browser-engine.server-CGd-YOU2.mjs");
...
var browseScrollFn = createServerFn({ method: "POST" }).validator(object({
	...
	const { browseScroll } = await import("./browser-engine.server-CGd-YOU2.mjs");
...
var browseCheckIpFn = createServerFn({ method: "POST" }).validator(SessionInput).handler(browseCheckIpFn_createServerFn_handler, async ({ data }) => {
	const { browseCheckIp } = await import("./browser-engine.server-CGd-YOU2.mjs");
...
var closeSessionFn = createServerFn({ method: "POST" }).validator(object({ profileId: string() })).handler(closeSessionFn_createServerFn_handler, async ({ data }) => {
	const { closeSession } = await import("./browser-engine.server-CGd-YOU2.mjs");
...
var pickLiveProxiesFn = createServerFn({ method: "POST" }).validator(object({
	...
	const { pickLiveProxies } = await import("./proxy-tunnel.server-C0Jwv65i.mjs");
